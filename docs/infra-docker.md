...
| TimescaleDB | 5432 | Ticks e barras OHLC (M15 + M1) |
...
1. **Bootstrap**: `sync_all_symbols_to_triton` copia `latest_ts.pt` → `{symbol}/1/model.pt` + `config.pbtxt`.
...
4. **Produção**: `TritonGrpcClient` mantém canal `grpc.aio.insecure_channel` persistente, inferências paralelas (`asyncio.gather`) e timeout configurável (`infra.triton.infer_timeout_seconds`, padrão **0,85 s**).
...
Dimensão canônica no app: **`META_FEATURE_DIM = 43`** (34 TCN + 4 micro-vol + 3 cross + 2 flow). O artefato `.pkl` e o treino offline (`train_meta_*.py`) devem alinhar com essa dimensão.
