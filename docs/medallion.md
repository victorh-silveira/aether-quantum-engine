...
| `confidence_call_threshold` | 0.57 | Threshold CALL (piso Kelly / BE + margem) |
| `confidence_put_threshold` | 0.43 | Threshold PUT |
...
**Justificativa do score 0.52 em squeeze**: em canais Bollinger esmagados, o rebaixamento marca `meta_squeeze_downgrade` e, **fora de recovery financeira** (`pending_total == 0`), comprime a stake ao piso de $1.00 da Deriv — **exceto** Soft_SIZE com Edge >= **0.015** (`d_squeeze_floor_waived_for_soft_size`), que preserva o piso Soft_SIZE **2.5%**. Com `pending_total > 0`, o piso D-SQUEEZE e **revogado** (`d_squeeze_floor_waived_for_recovery`) para preservar a stake soft D'Alembert e cobrir o passivo; o ranking de recovery tambem penaliza simbolos em squeeze quando ha peer elegivel. O adaptativo `bb_width_adaptive_squeeze` permanece **desabilitado** nos settings atuais.
