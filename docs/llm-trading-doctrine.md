...
O LLM/Cursor e **copiloto de engenharia e auditoria**. Nao decide CALL/PUT em runtime. A decisao live permanece TCN + meta LightGBM + Kelly/caps + **fusao EV multi-escala** (`execution_direction_fusion`); **escopo 1.1** + arquitetura continua R_10: catálogo `signal_skip` (`mini_pair_oppose` / `cal_margin` / chop = soft Kelly **0.55**); **neg_edge** le Edge Cal TCN do lado (`CLUSTER`, nao `fusion_p_eff`); soft se `0 < Cal < min_edge_execute` (`neg_edge_hard_skip` **false**, `neg_edge_soft_min_edge` **−1.0**); **hard** se Cal `<= 0` (`fusion_p_eff` nao libera EXEC); sob seed, soft mult **0.25** e hard `boot_deep` se edge &lt; `neg_edge_deep_edge_floor` (**−0.12**); loss-clf soft na faixa media; **FLIP** CALL↔PUT **relativo ao TCN** se `p_loss >= 0.90` e `veto_ready` e `flip_require_auto_learn` **true** (seed so SOFT; `p_ovr`/`seed_discord` nao FLIP com `auto=0`; seed: `flip_seed_block_against_closed_candle` + `flip_seed_waive_edge_min` **−0.08**; live: `flip_waive_edge_min` **−1.0**; telemetria `FLIP_BLOCK:*`; `flip_waive_tcn_pos_edge_on_discord` **true**); fusao `scale_vision.fusion_*` (`fusion_meta_ev_weight` **0.10**, `fusion_loss_weight` **0.45**, `fusion_tcn_shrink_near_half` **0.25**, `fusion_block_when_tcn_pos_edge` **true**, `fusion_block_when_tcn_candle_agree` **true**, `fusion_loss_requires_auto_learn` **true**, `fusion_loss_seed_weight_mult` **0.0**, soft EV fraco `fusion_weak_ev_soft_kelly_mult` **0.40**, seed+ambos EV&lt;0 `fusion_weak_ev_seed_soft_kelly_mult` **0.25**, log `[GATES] || FUSION`) escolhe lado por argmax EV — seed **nao** alimenta `fusion_loss_weight`; se TCN==janela ops N=5 e fusao tentaria inverter → `why=tcn_candle_agree` — sem revenge sizing pos-LOSS e sem reabrir quality gate amplo (RSI/price_zone/SIDE_EQ block). **Anti-loss** (`anti_loss_*`): **seed** unstamped + `p_loss`≥**0.85** + TCN pos_edge → **hard SKIP** se a janela nao confirma TCN com corpo minimo **0.10**; **live** (janela stampada, mesmo `auto=0`): tier fraco (corpo liquido &lt; **0.10** ou ausente) + SKIP se exec≠janela + gate unificado SKIP se a janela nao confirma o **lado EXEC** com corpo &gt;= **0.75**; `anti_loss_seed_discord`; soft so se `anti_loss_hard_skip` **false**. **M1 last-bar = log; confirmacao = janela N=5.** `invert_exec_side` **false**; `online_training` **false**; stop-win **3%** composto = metrica de sessao. EMPTY de sinal `neg_edge` se Cal TCN `<= 0` (ou seed profundo / override) ou `anti_loss_seed_discord`; EMPTY tecnico permanece.

SSOT operacional: [`config/settings.json`](../config/settings.json) — universo **`R_10`** (Volatility 10), contrato ops **5 m (M5)**, label TCN **N** ∈ {15,20,…,60} eleito no launch-train (**SSOT atual N=55**), micro/MINI **60 s** (M1), ciclo **60 s**, macro **7200 s**. Metodologia: [`medallion.md`](medallion.md). Sample size: [`sample-size-lln.md`](sample-size-lln.md). Loss ML: [`infra-docker.md`](infra-docker.md) (`aether-loss-classifier`).
...
4. **KELLY** — `mode=explore|recover`, `live_n`, `f*`, `kelly_fraction_scale` (inclui SIDE_EQ soft). Com fusao, `p` rastreia `fusion_p_eff` **so se** Cal TCN ja passou `neg_edge` (nao breakeven se `p_eff` alto). Cold start: `explore_stake_scale_floor` **0.40**. RECOVER: stake = cover pleno (`cover_multiple` **1.50**, amort **1/1**); `f*` so gate. Se `[RESOLVED] PEND` nao cai apos WIN → regressao de audit.
