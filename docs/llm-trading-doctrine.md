...
O LLM/Cursor e **copiloto de engenharia e auditoria**. Nao decide CALL/PUT em runtime. A decisao live permanece TCN + meta LightGBM + Kelly/caps + **fusao EV multi-escala** (`execution_direction_fusion`); **escopo 1.1** + arquitetura continua R_10: catálogo `signal_skip` (`mini_pair_oppose` / `cal_margin` / chop = soft Kelly **0.55**); **neg_edge** soft se `0 < edge < min_edge_execute` (`neg_edge_hard_skip` **false**, `neg_edge_soft_min_edge` **−1.0**); **hard** se `edge <= 0`; sob seed, soft mult **0.25** e hard `boot_deep` se edge &lt; `neg_edge_deep_edge_floor` (**−0.12**); loss-clf soft na faixa media; **FLIP** CALL↔PUT **relativo ao TCN** se `p_loss >= 0.90` e `veto_ready` e `flip_require_auto_learn` **true** (seed so SOFT; seed: `flip_seed_block_against_closed_candle` + `flip_seed_waive_edge_min` **−0.08**; live: `flip_waive_edge_min` **−1.0**; telemetria `FLIP_BLOCK:*`); fusao `scale_vision.fusion_*` (`fusion_meta_ev_weight` **0.10**, `fusion_loss_weight` **0.45**, `fusion_tcn_shrink_near_half` **0.25**, `fusion_block_when_tcn_pos_edge` **true**, `fusion_block_when_tcn_candle_agree` **true**, `fusion_loss_requires_auto_learn` **true**, `fusion_loss_seed_weight_mult` **0.0**, soft EV fraco `fusion_weak_ev_soft_kelly_mult` **0.40**, seed+ambos EV&lt;0 `fusion_weak_ev_seed_soft_kelly_mult` **0.25**, log `[GATES] || FUSION`) escolhe lado por argmax EV — seed **nao** alimenta `fusion_loss_weight`; se TCN==vela e fusao tentaria inverter → `why=tcn_candle_agree` — sem revenge sizing pos-LOSS e sem reabrir quality gate amplo (RSI/price_zone/SIDE_EQ block). **Anti-loss seed discord** (`anti_loss_*`): sob seed + `p_loss`≥**0.85** + TCN pos_edge → **hard SKIP** EXPLORE e RECOVER se a vela do `[CANDLE]` nao confirma o TCN com corpo minimo **0.10** (discord/DOJI/ruido); `anti_loss_seed_discord`; soft so se `anti_loss_hard_skip` **false**. `invert_exec_side` **false**; `online_training` **false**; stop-win **3%** composto = metrica de sessao. EMPTY de sinal `neg_edge` se `edge <= 0` (ou seed profundo / override) ou `anti_loss_seed_discord`; EMPTY tecnico permanece.
