...
Telemetria: `SCALE: … adapted=0|1 why=retract_vs_tcn|explos_vs_tcn|tape_vs_tcn|candle_vs_tcn|flip_holds|explos_edge_firm …`. `candle_vs_tcn` = alinhou ao `[CANDLE]`; `flip_holds` = manteve FLIP; `blocked=candle_holds` no GATES = FLIP bloqueado porque vela confirma TCN. `META: applied=1 edge=-1` → soft Kelly (`meta_soft_kelly`). Em bootstrap: `blocked=bootstrap boot=N/4`; `blocked=flip_min_n` se `n<8`. Edge CLUSTER = EV vs BE. `[CANDLE]` same-cycle = vela fechada; ciclo seguinte = janela do contrato anterior.

Algoritmo mental: `lado=TCN(Cal)` → FLIP so se pe no piso e vela≠TCN → senao SCALE regime(mi==mili)→tape_strong→candle last (FLIP sticky) → Kelly/META so sizing.
...
| FLIP | auto_learn; `n_train>=8`; `p_eff` no piso; **nao** se `[CANDLE]` == TCN (`candle_holds`) → oposto do TCN |
