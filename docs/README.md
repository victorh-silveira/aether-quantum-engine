...
| [structure.md](structure.md) | Layout do repositório e inventário de módulos Python em `app/src/` (~224) |
...
| DL | TCN, lookback **72**, macro **600 s**, `FEATURE_DIM=34`, label `ma_trend`, tensor `[1, 72, 34]` |
...
| Calibração | `neutral_half_width: 0.0` (zona neutra OFF); thresholds **0.50/0.50**; override TCN macro se raw &gt;0.65 ou &lt;0.35 |
| Indicator gating | enabled; `adx_min` 0.20; `vol_ratio_min` 0.65; `veto_on_noise` false |
| Quality gate | Dual soft + HARD microestrutura (`min_adx_threshold` 0.20; margins 0.0; `mandatory_min_trade_score` 0.50; `min_validation_accuracy_gate` 0.63) |
| BB squeeze adaptativo | **desabilitado** |
| Loss protection | Hard filters efetivamente desligados (margins 0.0, caps 999) |
...
| QA | Pre-commit: lint + testes 100% cobertura (**284** `test_*.py`) + security |
