...
| `app/src/application/services/execution_scale_*.py` | Scale vision + retract/explos/tape/candle adapt (FLIP sticky) |
| `app/src/application/services/loss_classifier_*.py` | Loss-clf FLIP por p_eff (auto_learn apos exit live N=4; `n_train>=8`; young/mature pe>=0.58; `candle_holds`) |
...
| `app/src/application/services/execution_*.py` | Execution: tecnico + TCN + loss_clf FLIP + SCALE + Kelly |
