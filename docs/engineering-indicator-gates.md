...
2. SKIP sinal: `neg_edge` se Cal Edge ≤ 0 (`skip_neg_edge` **true**); waived com PEND ≥ `material_pending_min` ou smart waive (Edge >= -0.030 com loss_clf p_loss <= 0.485 ou loss-clf sob bootstrap com margem direcional ativa); `acc_floor` so quando `skip_below_soft_min_acc` **true** (ops **false**); `trend_discord` (tendencia e vela M5 discordam simultaneamente em EXPLORE); confluencia de mercado senior (`skip_exhaustion`, `skip_chop_congestion`, `skip_directional_momentum_discord`, `skip_two_bar_counter_trend`); salvaguardas de price action e fluxo (`skip_wick_rejection`, `skip_climactic_blowoff`, `skip_opposing_marubozu`, `skip_adverse_tick_flow`) protegendo contra falsos rompimentos, climax de exaustao, marubozus opostos sem absorcao e aceleracao contraria de micro-ticks.
