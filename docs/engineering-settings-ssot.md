...
| `kelly.neutral_bankroll_pct` | `risk_management.kelly` | Piso operacional de stake explore (**0.25%** banca M5); loss_clf soft **nao** esmaga o piso |
| `kelly.payout_fallback` / `params.payout_estimate` / `default_payout` | `risk_management` | Payout Deriv **1HZ75V** **0.85** (live; cover RECOVER = `cover_multiple * pending/0.85`) |
...
| `soft_recovery.amort_cycles_min` / `amort_cycles_max` | `risk_management.soft_recovery` | Amort **2/3**; stake RECOVER = `cover_multiple * pending/payout` (cover equilibrado; sem progressao exponencial; `f*` so gate) |
| `infra.loss_classifier.soft_max_stake_pct_high` | `infra.loss_classifier` | Teto stake EXPLORE sob soft (**5%**, alinhado a Single-Strike / `max_stake_pct`); waivado com pending material e com `FLIP_BLOCK` (keep TCN so atenua f*); ACC baixo nao cancela cover |
