...
| `signal_skip.*` | `orchestrator.execution` | Escopo **1.1**: mini/cal/chop soft Kelly **0.55**; **neg_edge_hard_skip** **false**; soft so se `0 < edge < min_edge_execute` (`neg_edge_soft_min_edge` **-1.0**); **hard** se `edge <= 0`; sob seed soft mult **0.25** + hard `boot_deep` se edge &lt; `neg_edge_deep_edge_floor` (**-0.12**); Edge = `Cal*(1+b)-1`; floor **0.04** exige Cal ≳ **0.605** para edge positivo; **anti-loss** seed+discord (`anti_loss_hard_skip` **true**): EXPLORE e RECOVER → `gate_reason=anti_loss_seed_discord` se a vela do `[CANDLE]` nao confirma o TCN com corpo minimo `anti_loss_min_candle_body` **0.10**; soft so se hard_skip **false** (`anti_loss_soft_kelly_mult` **0.25**) |
