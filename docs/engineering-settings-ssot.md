...
| Bloco | Papel |
|-------|--------|
| `orchestrator.execution` | mandatory/force off; `invert_exec_side` **false**; `skip_neg_edge` **true**; `skip_exec_vs_candle` / `skip_scale_candle_discord` / `skip_doji` **false**; `skip_below_soft_min_acc` **false**; settlement; SIDE_EQ soft sizing; `scale_vision` (adapt off); sample_size_policy; `post_loss_cooldown` (LIN>=1: 300/300/600/900s) |
...
| `infra.loss_classifier` | HTTP :8006; `veto_mode` **hard**; `hard_p_loss_floor` **0.58**; `flip_young_p_eff_floor` **0.51**; FLIP so apos auto_learn; `flip_min_n_train` **12**; `flip_trust_n` **32**; `flip_young_shrink` **0.50**; `ready_n` **32**; `retrain_min_n` **12**; saida do seed live **4** (`bootstrap_exit_n` / `LOSS_BOOTSTRAP_EXIT_N`, nao `ready_n`); `retrain_on_loss_min_n` / `min_win_for_loss_retrain` **4**; tape so telemetria; **sem** Soft Kelly / HARD SKIP |
...
- Loss-clf = **FLIP** young `p_eff` >=0.51 / mature >=0.58 (so apos auto_learn; `flip_young_shrink` **0.50**); tape so telemetria; nao HARD SKIP
