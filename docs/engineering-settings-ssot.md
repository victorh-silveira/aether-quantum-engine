...
| `signal_skip.anti_loss_allow_candle_flip` | `orchestrator.execution.signal_skip` | **true** — EMA/candle discord ou hybrid disagree + last ≠ EXEC → FLIP para lado da vela/last (soft) **so se** Edge Cal >= `min_edge_explore`/`min_edge_recovery` (**0.015** = **0.015**); subfloor → soft no anchor (`live_discord_weak` / `anti_loss_flip_blocked`); ao flipar sync `fusion_p_eff`; Edge final ≤0 → HARD neg_edge |
