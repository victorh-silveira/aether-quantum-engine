...
| `orchestrator.execution.meta_payoff_veto` | soft Kelly: edge ≤ 0 → `soft_veto_score_factor` **0.85**; edge ≤ **`soft_veto_strong_edge` -0.5** → `soft_veto_strong_factor` **0.40**; sat=1+Cal Edge≤**`soft_veto_sat_cal_edge_max` 0.02** também soft; **nao** re-eleva stake com `soft_size_min_stake_pct`; sem SKIP |
| `infra.meta_classifier` | HTTP :8005; `retrain_min_n` **32** |
| `risk_management` | Kelly Single-Strike 4.31% so com `live_n >= 12` e conviction ≥ **0.58**; `cover_enabled` **true** (`cover_multiple` **1.0**); piso **1%**; caps L0/L1 |
...
- Loss-clf = **FLIP** young `p_eff` >=0.58 / mature >=0.58 (so apos auto_learn); tape so telemetria; nao HARD SKIP
