...
| Bloco | Papel |
|-------|--------|
| `orchestrator.execution` | mandatory/force off; `invert_exec_side` **false**; confluencia senior nao e knob direcional; `skip_neg_edge` **true** sem smart waive em EXPLORE; `skip_exec_vs_candle` / `skip_scale_candle_discord` / `skip_doji` **false**; `skip_below_soft_min_acc` **false**; settlement; SIDE_EQ soft sizing; `scale_vision` (adapt off); sample_size_policy; `post_loss_cooldown` (LIN>=1: 300/300/600/900s) |
