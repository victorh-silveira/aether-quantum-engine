...
MACRO_COUNT="$("${COMPOSE[@]}" exec -T timescaledb psql -U "$PG_USER" -d "$PG_DB" -t -A -c "SELECT count(*) FROM ohlc_bars WHERE symbol='R_10' AND granularity=300;" 2>/dev/null || echo "0")"
...
if [ "$MICRO_COUNT" -lt 720 ] || [ "$MACRO_COUNT" -lt 80 ]; then
  docker_ui_warn "fome de dados (micro60=${MICRO_COUNT} macro300=${MACRO_COUNT}) - hidratando R_10"
  "${COMPOSE[@]}" exec -T timescaledb psql -q -U "$PG_USER" -d "$PG_DB" -c "
    ...
    FROM (SELECT NOW() - (i * INTERVAL '60 seconds') AS t, i FROM generate_series(1, 800) i) s
    ON CONFLICT DO NOTHING;
    INSERT INTO ohlc_bars (time, symbol, epoch, granularity, open, high, low, close)
    SELECT t, 'R_10', EXTRACT(EPOCH FROM t)::bigint, 300,
           100.0+(i*0.02), 100.8+(i*0.02), 99.2+(i*0.02), 100.2+(i*0.02)
    FROM (SELECT NOW() - (i * INTERVAL '300 seconds') AS t, i FROM generate_series(1, 160) i) s
    ON CONFLICT DO NOTHING;" >/dev/null
  docker_ui_ok "lookback macro/micro reidratado"
else
  docker_ui_ok "ohlc_bars R_10 (micro60=${MICRO_COUNT} macro300=${MACRO_COUNT})"
fi
