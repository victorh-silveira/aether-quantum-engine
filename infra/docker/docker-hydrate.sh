...
MICRO_COUNT="$("${COMPOSE[@]}" exec -T timescaledb psql -U "$PG_USER" -d "$PG_DB" -t -A -c "SELECT count(*) FROM ohlc_bars WHERE symbol='OTC_SPC' AND granularity=900;" 2>/dev/null || echo "0")"
MICRO_COUNT="$(echo "$MICRO_COUNT" | tr -d '[:space:]')"
MACRO_COUNT="$("${COMPOSE[@]}" exec -T timescaledb psql -U "$PG_USER" -d "$PG_DB" -t -A -c "SELECT count(*) FROM ohlc_bars WHERE symbol='OTC_SPC' AND granularity=3600;" 2>/dev/null || echo "0")"
...
if [ "$MICRO_COUNT" -lt 720 ] || [ "$MACRO_COUNT" -lt 80 ]; then
  docker_ui_warn "fome de dados (micro900=${MICRO_COUNT} macro3600=${MACRO_COUNT}) - hidratando OTC_SPC M15"
  "${COMPOSE[@]}" exec -T timescaledb psql -q -U "$PG_USER" -d "$PG_DB" -c "
    ...
    SELECT t, 'OTC_SPC', EXTRACT(EPOCH FROM t)::bigint, 900,
           100.0+(i*0.01), 100.5+(i*0.01), 99.5+(i*0.01), 100.1+(i*0.01)
    FROM (SELECT NOW() - (i * INTERVAL '900 seconds') AS t, i FROM generate_series(1, 800) i) s
    ON CONFLICT DO NOTHING;
    INSERT INTO ohlc_bars (time, symbol, epoch, granularity, open, high, low, close)
    SELECT t, 'OTC_SPC', EXTRACT(EPOCH FROM t)::bigint, 3600,
  ...
  docker_ui_ok "lookback macro/micro M15 reidratado"
else
  docker_ui_ok "ohlc_bars OTC_SPC (micro900=${MICRO_COUNT} macro3600=${MACRO_COUNT})"
fi
