...
- Env: `LOSS_READY_N` **32** (`veto_ready`/maduro), `LOSS_RETRAIN_MIN_N` **12**, `LOSS_BOOTSTRAP_EXIT_N` **12** (saida do seed / auto_learn; nao `READY_N`), `LOSS_MIN_WIN_FOR_LOSS_RETRAIN` **4**, `LOSS_VETO_P_LOSS_FLOOR` **0.58** (telemetria no sidecar; motor ignora para SKIP)
