...
8. RECOVER vs EXPLORE; pending/cover amortizado (`cover_multiple` **1.50**, amort **2–4**, linear3 teto **2.5%**); damping stop-win inicio **1.0** / perto-meta **0.50**; RECOVER stake = cover×damping (`f*` so gate); explore piso **0.25%** banca + `explore_stake_scale_floor` **0.40**; Kelly `fraction` **0.08**; caps stop-win Kelly ate **5%**. Se `f*≈0` e stake ≈ 1ª Kelly da sessao sob soft/freeze → regressao **U sticky** (usar piso/cover, nao `dlambert_unit`)
9. EXEC_EMPTY tecnico = sucesso de processo quando coerente; soft sinal (incl. neg_edge) continua EXEC — EMPTY de sinal `neg_edge` so seed profundo ou override; soft continua EXEC mas tamanho ≠ revenge via U sticky
