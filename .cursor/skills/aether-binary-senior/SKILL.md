...
Universo: **Volatility 75 (1s) Index** (`1HZ75V`) — **M5** (contrato ops **5 m / M5**; label TCN **N=1 vela M5**; ciclo **300 s** com `require_signature_boundary` **true**; micro/MINI **300 s**; macro **86400 s**; ratio **1:288**).
...
7. Chop = soft Kelly; **neg_edge** com `neg_edge_hard_skip` **true** (SSOT): Edge `<= 0` → HARD (`NEG_EDGE nonpos` / `boot_deep` se seed+Cal < **−0.12**); Edge `< min_edge_*` (**0.015** explore = recovery) → HARD (`neg_edge_subfloor_hard`); Soft_SIZE so soft flags **com** Edge >= floor (**sem** Single-Strike); **nao** Soft_SIZE para `0 < Edge < floor`; **trava de pânico Z-score bilateral**: CALL vetado se $Z < -2.0$, PUT vetado se $Z > +2.0$ (`gate_reason=neg_edge_zscore_panic`; telemetria `Z=`/`side`/`thr`)
...
10. EXEC_EMPTY tecnico = sucesso quando Edge `<= 0` / Edge `< floor` / `neutral_zone` / seed unstamped; soft EMA/confirm/RSI continua EXEC sem Single-Strike (piso Soft_SIZE **2.5%** so se Edge >= **0.015**)
## Proibido

- Revenge sizing pos-LOSS; force_trade como fix de EXEC_EMPTY / Edge<=0 / Edge < floor / neutral_zone
- Julgar EMPTY Edge<=0 ou Edge < floor como bug de frequencia; reabrir `neg_edge_hard_skip`; Soft_SIZE sob Edge subfloor
- Soft_SIZE + Edge forte com stake &lt; 1%; Soft_SIZE+PEND+Edge>=0.015+stake≈1%; EXEC PUT com candle CALL sem flip; EXEC stake &lt; 1% banca; PEND + stake ≈ cover amort
