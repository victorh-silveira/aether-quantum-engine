...
5. **Micro protect**: vela M5 ≠ EXEC + corpo minimo → HARD `micro_discord` (sem voto tape); soft/FLIP_BLOCK + `p_loss`>=**0.80** (qualquer regime) → HARD `chop_loss_risk`; Soft so EXEC se `confirm_score` >= **2** (vela/tape/mi/mili/ops definidos); senao HARD `soft_confirm_weak` (sem flip)
