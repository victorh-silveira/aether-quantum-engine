...
6. Loss-clf: soft vs FLIP (`p_loss>=0.90`); **nunca FLIP** se Edge TCN >= floor (`tcn_edge`); `p_loss>=0.95` override seed/SCALE (`p_ovr`); vela no alvo floor **0.85** so se TCN fraco; soft edge so se >= **-0.05** e sem FLIP_BLOCK
