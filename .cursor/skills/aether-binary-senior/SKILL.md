...
9. **Anti-loss seed discord**: seed + `p_loss`≥**0.85** + vela≠TCN + TCN pos_edge → **hard SKIP** EXPLORE e RECOVER (`gate_reason=anti_loss_seed_discord`); soft so se `anti_loss_hard_skip` **false** (`[GATES] || ANTI_LOSS`)
