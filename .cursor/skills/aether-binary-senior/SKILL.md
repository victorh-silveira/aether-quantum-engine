...
9. **Anti-loss seed discord**: seed + `p_loss`≥**0.85** + TCN pos_edge → **hard SKIP** EXPLORE e RECOVER se a vela do `[CANDLE]` nao confirma o TCN com corpo minimo **0.10** (`gate_reason=anti_loss_seed_discord`); soft so se `anti_loss_hard_skip` **false** (`[GATES] || ANTI_LOSS`)
