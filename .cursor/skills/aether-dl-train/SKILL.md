...
1. Settings: lookback **720**, micro **60s**, contrato **30s** (hibrido), `label_mode=ma_trend`, `deploy_gate.enabled` / `force_ok`
...
6. Checkpoint: feat_dim=34, lookback **720**, granularity micro **60**, `val_accuracy`, `deploy_ok`
...
12. Meta: variance nula → Timescale flat; usar `--source auto` / Deriv; alvo payoff fallback; gran meta **60s**
13. Triton/meta HTTP opcionais — confirmar flags
14. Pos-migrate 30s: invalidar pth/TorchScript antigos (gran 120), re-hydrate Timescale 60/300, retreinar TCN+meta

## Anti-padroes

Trocar label sem retreino; `force_ok=true`; treinar meta em hydrate sintetico; ignorar ACC; restaurar checkpoint so por loss; tratar vies de classe com veto de sinal live em vez de balance/recency/collapse; baixar `min_oos_sharpness` para “passar” export; tratar `[SUCESSO]` do bat se o gate/treino falhou.
