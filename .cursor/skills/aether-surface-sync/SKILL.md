...
6. **Anti-sujeira** — apagar `_tmp*`, `COMMIT_MSG.txt`, probes locais; grep por refs stale ao path/simbolo removido; sem imports mortos
7. **Higiene pontual** — se houver morto comprovado no diff, seguir `aether-repo-hygiene` (evidencia antes de delete)
8. **Pre-commit** — WSL + Conda `deriv-api`: `make app-pre-commit-run` (ou commit com hooks). Falha → skill `aether-precommit`
9. **Entrega** — commits PT-BR; nao deixar superficie agentes desatualizada “para depois”
...
`docs/engineering-surface-sync.md`, `docs/agent-coverage.md`, `AGENTS.md`, skills `aether-precommit` + `aether-repo-hygiene`
