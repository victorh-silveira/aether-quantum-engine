...
6. Loss-clf: `veto_mode` **hard**; piso **0.55** (`hard_p_loss_floor` + `flip_young_p_eff_floor`); FLIP so apos auto_learn; `flip_trust_n` **32**; tape so telemetria; **proibido** Soft Kelly / HARD SKIP no piso / bloco `signal_skip` / `invert_exec_side` / fusao de lado
