...
6. Loss-clf: `veto_mode` **hard**; mature **0.58** (`hard_p_loss_floor`); young **0.55** (`flip_young_p_eff_floor`); FLIP so apos auto_learn; `bootstrap_exit_n` **4**; `flip_trust_n` **32**; `ready_n` **32**; `retrain_min_n` **12**; tape so telemetria; **proibido** Soft Kelly / HARD SKIP no piso / bloco `signal_skip` / `invert_exec_side` / fusao de lado
...
8. Edge CLUSTER = EV payout **0.85** (telemetria; **sem** knobs `cal_soft_edge_*`)
9. SCALE: `adapt_retract_enabled` **true** (retract/explos last apos FLIP; sem `adapt_soft_margin`)
10. META: edge ≤ 0 → soft Kelly (`soft_veto_score_factor` **0.85**; edge ≤ **-0.5** → `soft_veto_strong_factor` **0.40**; sem SKIP / sem `meta_veto_mode=soft`)
11. Nao reintroduzir quality gate amplo / price_zone / `hard_cal_margin_floor`
