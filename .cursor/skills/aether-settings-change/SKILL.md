...
6. Nao afrouxar doutrina de lado/sizing: `flip_*` (ex.: `flip_require_auto_learn`, `flip_seed_*`, `flip_waive_tcn_pos_edge_on_discord`), `fusion_*` (ex.: `fusion_block_when_tcn_pos_edge`, `fusion_block_when_tcn_candle_agree`, `fusion_loss_requires_auto_learn`, soft **0.40/0.25**, `fusion_loss_seed_weight_mult` **0.0**), `ops_window_bars` **5**, `neg_edge_*` (ex.: `neg_edge_deep_edge_floor` **−0.12**), `anti_loss_hard_skip` (**true**), `anti_loss_min_candle_body` (**0.10**), `anti_loss_live_weak_candle_enabled` (**true**), `anti_loss_live_confirm_enabled` (**true**), `anti_loss_live_confirm_min_body` (**0.75** no corpo liquido das 5 M1), `anti_loss_live_exec_candle_enabled` (**true**), `invert_exec_side` (**false**), `online_training` (**false**), caps recovery (amort **1/1**, cover **1.50**, linear3 **2.5%**)
