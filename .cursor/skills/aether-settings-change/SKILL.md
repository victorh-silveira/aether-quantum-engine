...
7. Sizing: `cover_enabled` **true**, amort **1/1**, stake = `min(max(PEND/payout, 1% banca), cap_L0)`; cover L0 **3.5%**; PEND nao force-explore por near-stop; piso Kelly **1%** soberano, Single-Strike 4.31% so com `live_n >= 12` e conviction ≥ **0.58**; `online_training` **false**; sem pausa temporal pós-LOSS; `counter_trend_min_edge` **0.08**
