...
def resolve_dual_granularity(data_config: dict) -> tuple[int, int]:
    ...
    micro_raw = int(data_config.get("micro_granularity", data_config.get("cycle_granularity", 180)))
    micro = normalize_granularity_seconds(micro_raw)
    return macro, micro


def resolve_mini_granularity(data_config: dict) -> int:
    """Resolve granularidade MINI (tape curto; default 180s M3)."""
    raw = int(data_config.get("mini_granularity", 180))
