...
def _attempt_bull_call_lock_flip(
    ...
    *,
    ...
    bull_call_prob = entry_prob(peer_entry)

    bear_put_prob = 1.0 - entry_prob(entry)
    ...
    resolved_dir, action = evaluate_anti_trend_lock(
        ...
        bull_call_prob=bull_call_prob,
        bear_put_prob=bear_put_prob,
        probability_delta=probability_delta,
        predicted_payoff_edge=predicted_payoff_edge,
        cross_symbol_prob_delta_mean=cross_symbol_prob_delta_mean,
    )
...
def _attempt_bear_put_lock_flip(
    ...
    *,
    ...
    bull_call_prob = entry_prob(entry)

    bear_put_prob = 1.0 - entry_prob(peer_entry)
    predicted_payoff_edge = peer_payoff_edge(entry, metrics)
    cross_symbol_prob_delta_mean = cross_prob_delta_mean(metrics, infra_cfg)
    probability_delta = cross_symbol_conviction_spread(metrics)
