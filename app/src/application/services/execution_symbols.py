...
from src.application.services.execution_symbols_recovery import pending_recovery_active, recovery_candidate_pool
...
def select_best_execution_candidate(
    ...
    *,
    ...
    """Seleciona o melhor candidato do pool ranqueado."""
    pool = recovery_candidate_pool(
        candidates, last_loss_symbol=last_loss_symbol, recovery_active=recovery_active, skip_symbols=skip_symbols
    )
    if not pool:
        return None

    def rank_key(item: tuple[str, TradeDirection, dict]) -> float:
        """Chave de ranking por score de execucao."""
...
def select_mandatory_execution_candidate(
    ...
    *,
    ...
    """Seleciona candidato obrigatorio a partir do pool de recovery."""
    pool = recovery_candidate_pool(
        candidates, last_loss_symbol=last_loss_symbol, recovery_active=recovery_active, skip_symbols=skip_symbols
    )
    if not pool:
    ...
    return select_best_execution_candidate(
        pool, last_loss_symbol=last_loss_symbol, recovery_active=recovery_active, skip_symbols=skip_symbols
    )


def format_execution_alternates(
    candidates: list[tuple[str, TradeDirection, dict]], *, exclude_symbol: str | None = None, limit: int = 2
) -> str:
    """Formata simbolos alternativos ordenados por score para log EXEC."""
    ranked = sorted(
        candidates, key=lambda item: candidate_execution_score(item[2], recovery_active=False), reverse=True
    )
