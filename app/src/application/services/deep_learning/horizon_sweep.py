"""Sweep de horizonte N barras no TF SSOT (R_10 M3)."""
...
DEFAULT_N_BARS: tuple[int, ...] = (1, 2, 3, 5)
M3_SECONDS = 180


def duration_minutes_for_n(n_bars: int, *, micro_seconds: int = M3_SECONDS) -> int:
...
def parse_n_bars(raw: Any) -> tuple[int, ...]:
    """Normaliza a grade de N; vazio cai no default 1,2,3,5."""
...
def load_horizon_sweep_knobs(settings: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "n_bars": list(parse_n_bars(block.get("n_bars"))),
...
def build_horizon_candidates(
    ...
    *,
    ...
    bars = parse_n_bars(n_bars if n_bars is not None else h_block.get("n_bars"))
