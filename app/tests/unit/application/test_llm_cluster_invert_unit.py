...
def test_cluster_propagate_inverts_when_z_blocks_put():
    orch = MagicMock()
    orch.anchor = "frxEURUSD"
    ...
    lines = [str(c) for c in orch.logger.info.call_args_list]
    assert any("CLUSTER_INVERT" in line for line in lines)
    assert any("CLUSTER_BEST" in line or "CLUSTER_PROP" in line for line in lines)
...
def test_cluster_propagate_logs_block_when_hmm_vetoes():
    ...
    assert decisions["OTC_FCHI"]["metrics"]["llm_exec_inverted"] is True
    lines = [str(c) for c in orch.logger.info.call_args_list]
    assert any("CLUSTER_INVERT" in line for line in lines)
