...
async def test_place_order_retries_lower_stake(orch_config):
    ...
    with patch(
        ...
        contract = await place_order(executor, "OTC_SPC", TradeDirection.CALL, 100.0)
...
async def test_place_order_raises_after_all_retries(orch_config):
    ...
    with (
        ...
        await place_order(executor, "OTC_SPC", TradeDirection.CALL, 100.0)
...
async def test_execute_orders_registers_proposal_skip(orch_config):
    ...
    orders = [("OTC_SPC", TradeDirection.CALL, {"execute": True, "trade_score": 0.6})]
