...
def test_parse_micro_protect_ssot():
    ...
    assert float(cfg["chop_loss_risk_p_loss_floor"]) >= 0.85
...
def test_parse_micro_protect_rejects_bad_floor():
    try:
...
def test_micro_discord_noop_without_confirm():
    metrics = {
        "execution_candidate_ready": True,
        "signal_status": "CALL",
        "exec_direction": "CALL",
...
def test_chop_loss_risk_noop_when_not_chop():
    metrics = {
        "execution_candidate_ready": True,
        "signal_status": "CALL",
        "exec_direction": "CALL",
