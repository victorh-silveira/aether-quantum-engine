...
def test_neg_edge_soft_when_side_agrees_closed_candle():
    ...
    assert apply_negative_cal_edge_pause(metrics, orch=_orch_skip(neg_edge_hard_skip=False)) is False
...
def test_neg_edge_soft_when_flip_blocked_and_candle_with_hard_off():
    metrics = {
        "execution_candidate_ready": True,
        "exec_direction": "CALL",
        "calibrated_prob": 0.59,
...
def test_neg_edge_hard_subfloor_when_hard_skip_true():
    metrics = {
        "execution_candidate_ready": True,
        "exec_direction": "CALL",
        "calibrated_prob": 0.59,
