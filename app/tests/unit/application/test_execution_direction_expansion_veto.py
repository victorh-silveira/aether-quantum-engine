...
def test_expansion_veto_noop_when_exec_matches_dl():
    metrics = {"indicators": {"vol_ratio": 1.30}}
...
def test_resolver_trend_expansion_preserves_dl_on_high_vol_exhaustion():
    entry = _entry(
    ...
    result = resolve_execution_direction(
        entry,
        exec_cfg={"exhaustion_gate": {}, "regime_evaluator": {"enabled": True}},
    )
...
def test_resolver_compression_trap_inverts_when_stretched_in_expansion_band():
    entry = _entry(
        direction=TradeDirection.CALL,
        raw_prob=0.65,
        trend_direction=None,
