...
def test_get_data_state_signature_empty_without_stream():
    orch = SimpleNamespace(symbols=["OTC_SPC"], stream=None)
...
def test_dual_timeframe_data_signature(orch_ready):
    ...
    assert ";m5:" in sig
    assert ";m15:" in sig
    assert f"OTC_SPC@{_MACRO_EPOCH}" in sig
    assert f"OTC_SPC@{_MICRO_EPOCH}" in sig
...
def test_data_signature_changes_when_micro_epoch_advances_with_static_m15(orch_ready):
    ...
    orch.stream.micro_candles = {"OTC_SPC": [_bear_candle(_MICRO_EPOCH + 300)]}
    sig_after = get_data_state_signature(orch, now=float(_MICRO_EPOCH + 300))
    assert sig_before != sig_after
    assert f"OTC_SPC@{_MACRO_EPOCH}" in sig_before
    assert f"OTC_SPC@{_MACRO_EPOCH}" in sig_after
...
def test_get_data_state_signature_reads_macro_from_legacy_candles_store(orch_ready):
    ...
    orch.stream.candles = {"OTC_SPC": [_bear_candle(_MACRO_EPOCH)]}
    orch.stream.micro_candles = {"OTC_SPC": [_bear_candle(_MICRO_EPOCH)]}
