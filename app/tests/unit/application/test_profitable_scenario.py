...
def test_cluster_symbol_open_without_allowlist():
    assert cluster_symbol_allowed_for_tag(None, macro_tag="risk_on", symbol="OTC_SPC") is True


def test_normalize_maps_skip_invalid_entries():
    cfg = resolve_profitable_scenario_config(
        {
            "allowed_cluster_symbols_by_tag": {"": ["OTC_DJI"], "risk_on": []},
...
def test_resolve_profitable_scenario_config_merges_maps():
    cfg = resolve_profitable_scenario_config(
        {
            "allowed_cluster_symbols_by_tag": {"risk_off": ["OTC_FCHI"]},
            "min_conviction_by_tag": {"risk_off": 0.68},
            "confluence_conviction_floor": 0.65,
        }
    )
    assert cfg["allowed_cluster_symbols_by_tag"]["risk_off"] == ("OTC_FCHI",)
    assert cfg["min_conviction_by_tag"]["risk_off"] == 0.68


def test_cluster_symbol_allowlist_open_when_tag_missing():
    macro = {"allowed_cluster_symbols_by_tag": {"risk_on": ["OTC_DJI"]}}
    assert cluster_symbol_allowed_for_tag(macro, macro_tag="risk_off", symbol="OTC_FCHI") is True


def test_cluster_symbol_allowlist_blocks_wrong_index():
    macro = {
        "allowed_cluster_symbols_by_tag": {"risk_on": ["OTC_DJI"]},
        "allowed_execute_tags": ("risk_on",),
        "confluence_conviction_floor": 0.65,
        "assert_min_hmm_prob": 0.0,
    }
    assert cluster_symbol_allowed_for_tag(macro, macro_tag="risk_on", symbol="OTC_SPC") is False
    assert cluster_symbol_allowed_for_tag(macro, macro_tag="risk_on", symbol="OTC_DJI") is True
