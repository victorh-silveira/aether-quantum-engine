...
def test_maybe_pause_symbol_session():
    ...
    assert orch._dl_session_pause["R_10"] == 4
    assert is_symbol_session_paused(orch, "R_10") is True
    quiet = SimpleNamespace(_dl_outcome_flags={"R_10": [True, True, False]})
    ...
    maybe_pause_symbol_session(cfg_orch, "R_10", max_losses_in_window=2, window_trades=3, pause_cycles=2)
    assert cfg_orch._dl_session_pause["R_10"] == 2
    typed = SimpleNamespace(
        _dl_outcome_flags={"R_10": [False, False, False]},
        config={"orchestrator": {"cycle_interval_seconds": {}}},
    )
    maybe_pause_symbol_session(typed, "R_10", max_losses_in_window=2, window_trades=3, pause_cycles=2)
    assert typed._dl_session_pause["R_10"] == 2
    tick_dl_session_pauses(SimpleNamespace(_dl_session_pause="x", _dl_session_pause_until={"X": None}))
