...
def test_symbol_priority_recovery_core_only():
    order = _symbol_priority(["R_10", "R_50"], "R_10", recovery_core_only=True)
    assert order == ["R_10"]


def test_symbol_priority_reuses_core_when_tail_empty():
    order = _symbol_priority(["R_10"], "R_10", recovery_core_only=False)
    assert order == ["R_10"]
...
def test_forced_recovery_pick_skips_gate_blocked():
    ...
    assert _forced_recovery_pick(["R_10"], decisions, TradeDirection.CALL) is None
