...
def test_loss_clf_flip_above_floor_mature(monkeypatch):
    ...
    assert metrics["loss_clf_p_eff"] == 0.72
    assert metrics["loss_clf_flip_floor"] == 0.70
...
def test_loss_clf_ok_below_floor_mature(monkeypatch):
    ...
    _patch_predict(monkeypatch, p_loss=0.68, n_train=64)
    assert apply_loss_classifier_gate(metrics, TradeDirection.PUT, orch=_orch()) is False
    assert metrics["exec_direction"] == "PUT"
    assert metrics.get("loss_clf_flip") is not True
    assert metrics["loss_clf_p_eff"] == 0.68
    assert metrics["loss_clf_flip_floor"] == 0.70
...
def test_loss_clf_flips_even_when_candle_agrees_tcn(monkeypatch):
    metrics = {
        "tcn_direction": "CALL",
        "exec_direction": "CALL",
...
def test_loss_clf_young_high_p_flips(monkeypatch):
    ...
    _patch_predict(monkeypatch, p_loss=0.80, n_train=12)
    assert apply_loss_classifier_gate(metrics, TradeDirection.PUT, orch=_orch()) is False
    assert metrics["loss_clf_flip"] is True
...
def test_loss_clf_young_mid_p_no_flip(monkeypatch):
    ...
    _patch_predict(monkeypatch, p_loss=0.51, n_train=27)
    assert apply_loss_classifier_gate(metrics, TradeDirection.PUT, orch=_orch()) is False
    assert metrics.get("loss_clf_flip") is not True
    assert metrics["exec_direction"] == "PUT"
    assert metrics["loss_clf_p_eff"] < 0.58
