...
def test_force_retrain_and_clear():
    ...
    mark_force_retrain(orch, "OTC_SPC")
    ok, reason = should_retrain_symbol(orch, "OTC_SPC", {}, {"train_on_new_candle": False}, 1)
    assert ok and reason == "loss_retrain"
    clear_force_retrain(orch, "OTC_SPC")
    assert not (getattr(orch, "_dl_force_retrain", None) or {}).get("OTC_SPC")


def test_online_training_disabled_skips_all_runtime_retrain():
    orch = SimpleNamespace(_dl_force_retrain={"OTC_SPC": True}, _dl_bars_since_train={"OTC_SPC": 99})
    ...
    ok, reason = should_retrain_symbol(orch, "OTC_SPC", runtime, params, 1)
...
def test_deferred_train_pending_skips_retrain():
    ...
    orch = SimpleNamespace(_dl_deferred_tasks={"OTC_SPC": pending})
