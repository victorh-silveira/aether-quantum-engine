...
def test_resolve_deploy_ok_exige_mini_settlement():
    ...
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.54, val_brier=0.25, gate_cfg=cfg) is False
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.52, val_brier=0.25, gate_cfg=cfg) is False


def test_resolve_deploy_ok_force_ok_nao_promove_modelo_fraco():
    cfg = parse_deploy_gate_config({"deploy_gate": {"enabled": True, "force_ok": True}})
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.55, val_brier=0.30, gate_cfg=cfg) is False
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.40, val_brier=0.30, gate_cfg=cfg) is False
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.10, val_brier=0.99, gate_cfg=cfg) is False


def test_resolve_provisional_exige_anticolapso_e_habilitacao():
    cfg = parse_deploy_gate_config({"deploy_gate": {"provisional_enabled": True}})
...
def test_resolve_deploy_ok_mini_respeita_piso_anticolapso():
    cfg = {
        "enabled": True,
        "force_ok": False,
        "soft_min_val_accuracy": 0.53,
...
def test_resolve_deploy_ok_nao_tem_fallback_por_brier_global():
    cfg = parse_deploy_gate_config(
        {
            "deploy_gate": {
                "enabled": True,
    ...
    assert resolve_deploy_ok(mini_ok=False, val_accuracy=0.566, val_brier=0.250, gate_cfg=cfg) is False
...
def test_resolve_deploy_ok_rejects_majority_collapse():
    ...
    assert (
        resolve_deploy_ok(
            ...
            val_brier=0.24,
            gate_cfg=cfg,
            label_call_frac=0.51,
            pred_call_frac=0.24,
            minority_recall=0.26,
        )
        is False
    )
    assert (
        resolve_deploy_ok(
            ...
            val_brier=0.22,
            gate_cfg=cfg,
            label_call_frac=0.30,
            pred_call_frac=0.55,
            minority_recall=0.40,
        )
        is False
    )
    assert (
...
def test_describe_deploy_block_remaining_branches():
    ...
    assert "settlement OOS" in describe_deploy_block(mini_ok=False, val_accuracy=0.60, val_brier=0.1, gate_cfg=cfg)
