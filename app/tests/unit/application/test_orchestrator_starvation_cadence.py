...
def test_linear_edge_decay_limits():
    # Antes de 15 ciclos, o piso rígido é 0.01
    assert apply_starvation_edge_decay(0.04, 10) == pytest.approx(0.01)

    # No ciclo 15, o piso reduz para 0.01 - 0.05 = -0.04
    assert apply_starvation_edge_decay(0.04, 15) == pytest.approx(-0.04)

    # No ciclo 16, o piso reduz para 0.01 - 0.10 = -0.09
    assert apply_starvation_edge_decay(0.04, 16) == pytest.approx(-0.09)
