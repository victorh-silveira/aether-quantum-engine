...
def test_parse_regime_chop_from_ssot():
    ...
    assert cfg["chop_adx_max"] == pytest.approx(0.10)
    assert cfg["chop_hurst_min"] == pytest.approx(0.45)
    assert cfg["chop_hurst_max"] == pytest.approx(0.55)
    assert cfg["chop_soft_kelly_mult"] == pytest.approx(0.75)
    skip = parse_signal_skip_config({})
    assert skip["chop_pause_enabled"] is False
    assert skip["chop_soft_kelly_mult"] == pytest.approx(0.75)
