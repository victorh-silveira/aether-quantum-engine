...
async def test_enqueue_runs_training_in_background():
    ...
    with patch(
        ...
        task = orch._dl_deferred_tasks["R_75"]
        await task

    mock_thread.assert_awaited_once()
    assert "R_75" not in (getattr(orch, "_dl_force_retrain", None) or {})
...
async def test_enqueue_skips_when_task_pending():
    ...
    orch._dl_deferred_tasks = {"R_50": pending}
    train_fn = MagicMock()
...
async def test_enqueue_logs_training_failure():
    ...
    with (
        ...
        await orch._dl_deferred_tasks["R_25"]
