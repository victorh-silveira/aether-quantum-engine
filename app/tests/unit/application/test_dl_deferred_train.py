...
async def test_deferred_training_chains_next_bootstrap_symbol():
    ...
    with (
        ...
        await orch._dl_deferred_tasks["RDBEAR"]
...
async def test_enqueue_runs_training_in_background():
    ...
    with patch(
        ...
        task = orch._dl_deferred_tasks["RDBEAR"]
        await task

    mock_thread.assert_awaited_once()
    assert "RDBEAR" not in (getattr(orch, "_dl_force_retrain", None) or {})
...
async def test_enqueue_skips_when_task_pending():
    ...
    orch._dl_deferred_tasks = {"RDBULL": pending}
    train_fn = MagicMock()
...
def test_cancel_deferred_symbol_training():
    ...
    pending = MagicMock()
    pending.done.return_value = False
    orch._dl_deferred_tasks = {"RDBEAR": pending}
...
async def test_enqueue_defers_when_another_symbol_training():
    ...
    with patch(
        ...
        assert "RDBULL" not in orch._dl_deferred_tasks
        await orch._dl_deferred_tasks["RDBEAR"]
