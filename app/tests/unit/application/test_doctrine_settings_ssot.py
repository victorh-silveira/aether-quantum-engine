...
def test_production_loss_classifier_soft_veto_ssot():
    ...
    assert float(skip["anti_loss_soft_kelly_mult"]) == pytest.approx(0.75)
    ...
    assert int(data["fetch_count"]) == 365
    ...
    assert int(settings["risk_management"]["kelly"]["cycle_stake_baseline_seconds"]) == 300
    ...
    dl = settings["deep_learning"]
    assert int(dl["training_history_bars"]) == 365
    ...
    assert list(h_sweep["duration_minutes"]) == [5, 10, 15, 20]
