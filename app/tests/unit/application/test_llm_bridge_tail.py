...
async def test_fetch_context_blocks_returns_narrative_strings_and_context_extra():
    ...
    macro_d, struct_d, swing_d, trig_d, mtf, extra = await bridge.fetch_context_blocks(orch, "R_100", runtime)
    assert (
...
async def test_fetch_context_blocks_usa_ohlc_m1_quando_disponivel():
    ...
    _macro, _struct, _swing, _trig, _mtf, extra = await bridge.fetch_context_blocks(orch, "R_100", runtime)
...
def test_last_reference_price_reads_close_and_empty():
    ...
    assert last_reference_price(stream, "R_100") == 10.5
    stream.get_numpy_series.return_value = np.array([])
    assert last_reference_price(stream, "R_100") is None
