...
def test_post_loss_ban_blocks_repeat_direction():
    ...
    register_post_loss_ban(orch, "RDBEAR", TradeDirection.CALL, candle_cycles=2)
    assert post_loss_block_reason(orch, "RDBEAR", TradeDirection.CALL) == "repeat_loss"


def test_post_loss_tick_clears_ban():
    orch = SimpleNamespace(config={"deep_learning": {}})
    register_post_loss_ban(orch, "RDBEAR", TradeDirection.CALL, candle_cycles=1)
    tick_post_loss_bans(orch)
    assert post_loss_block_reason(orch, "RDBEAR", TradeDirection.CALL) is None


def test_apply_cooldown_sets_repeat_loss_gate():
    orch = SimpleNamespace(config={"deep_learning": {"post_loss_flip_raw_min": 0.99}})
    register_post_loss_ban(orch, "RDBEAR", TradeDirection.PUT, candle_cycles=2)
