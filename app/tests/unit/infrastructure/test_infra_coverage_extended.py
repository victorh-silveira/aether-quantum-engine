...
from src.infrastructure.handlers.stream_tick_sidecar import persist_closed_bar
...
async def test_minio_upload_creates_bucket(tmp_path):
    ...
    src = tmp_path / "a.pth"
    src.write_bytes(b"1")
    await store.upload("RDBEAR", src, arch="tcn", metadata={"x": 1})
...
async def test_minio_download_fail_and_head_false():
    ...
    with patch("asyncio.to_thread", new=AsyncMock(side_effect=[False, False])):
        assert await store.download_latest("RDBEAR", arch="tcn", dest=Path("x.pth")) is False
...
async def test_timescale_worker_batch_timeout():
    ...
    writer._closed = True
    writer._queue.put_nowait(("tick", (1, "RDBEAR", 1, 1.0)))
...
async def test_stream_handler_persist_bar_skips_without_writer():
    stream = StreamHandler(MagicMock(), ["RDBEAR"], {}, market_writer=None)
    candle = MagicMock(open=1.0, high=2.0, low=0.5, close=1.5)
    await persist_closed_bar(stream, "RDBEAR", 1, candle, None, granularity=900)
...
async def test_stream_handler_tick_and_bar_with_writer():
    writer = AsyncMock()
    stream = StreamHandler(MagicMock(), ["RDBEAR"], {}, market_writer=writer)
    await stream._on_tick({"tick": {"symbol": "RDBEAR", "epoch": 1, "quote": 1.5}})
    candle = MagicMock(open=1.0, high=2.0, low=0.5, close=1.5)
    micro = MagicMock(tick_count=2, mean_inter_tick_ms=1.0, price_velocity=0.1)
    await persist_closed_bar(stream, "RDBEAR", 10, candle, micro, granularity=900)
...
async def test_timescale_full_lifecycle():
    ...
    with patch("asyncpg.create_pool", AsyncMock(return_value=pool)):
        ...
        await writer.enqueue_tick(symbol="RDBEAR", epoch_ms=1000, price=1.0)
        await writer.enqueue_bar(symbol="RDBEAR", bar={"epoch": 1, "granularity": 900, "open": 1.0})
...
async def test_minio_download_raises(tmp_path):
    ...
    with patch("asyncio.to_thread", side_effect=_thread_run):
        assert await store.download_latest("RDBEAR", arch="tcn", dest=tmp_path / "x.pth") is False
    await store.close()
...
async def test_timescale_flush_bar_only():
    writer = TimescaleMarketWriter(dsn="postgresql://u:p@localhost/db")
    writer._queue.put_nowait(("bar", (1, "RDBEAR", 1, 900, 1, 1, 1, 1, 1, 1, 1)))
    with patch.object(writer, "_flush_batches", AsyncMock()) as flush_mock:
        await writer.flush()
