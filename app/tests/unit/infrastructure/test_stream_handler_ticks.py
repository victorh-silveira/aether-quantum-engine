...
async def test_stream_handler_on_tick_records_valid_tick(stream_handler):
    await stream_handler._on_tick({"tick": {"symbol": "R_10", "epoch": 1600000001, "quote": 100.25}})
    ticks = stream_handler.tick_buffer._live["R_10"]
    assert len(ticks) == 1
...
async def test_stream_handler_on_tick_ignores_invalid_payload(stream_handler):
    ...
    await stream_handler._on_tick({"tick": {"symbol": "R_10", "epoch": None, "quote": 1.0}})
    await stream_handler._on_tick({"tick": {"symbol": "R_10", "epoch": 1, "quote": None}})
    await stream_handler._on_tick({"tick": {"symbol": "R_10", "epoch": "bad", "quote": 1.0}})
    assert len(stream_handler.tick_buffer._live["R_10"]) == 0
