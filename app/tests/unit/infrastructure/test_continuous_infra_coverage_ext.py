...
def test_correlation_reader_log_returns_edges():
    ...
    short = compute_correlation_matrix({"OTC_SPC": [1.0, 1.01, 1.02], "R_50": [2.0, 2.01]})
    assert short[("OTC_SPC", "R_50")] == 0.0
    with patch(
        ...
        nan_matrix = compute_correlation_matrix(
            {"OTC_SPC": [1.0, 1.1, 1.2, 1.3, 1.4], "R_50": [2.0, 2.1, 2.2, 2.3, 2.4]}
        )
    assert nan_matrix[("OTC_SPC", "R_50")] == 0.0
