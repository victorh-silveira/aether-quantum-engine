...
def test_assert_triton_probability_bounds():
    assert assert_triton_probability(0.72, model_name="OTC_SPC") == 0.72
    with pytest.raises(RuntimeError, match="NaN"):
        assert_triton_probability(float("nan"), model_name="OTC_SPC")
    with pytest.raises(RuntimeError, match="fora"):
        assert_triton_probability(1.05, model_name="OTC_SPC")
...
async def test_triton_grpc_client_infer_2d_tensor():
    ...
    with (
        ...
        prob = await client.infer_symbol("OTC_SPC", np.zeros((4, 34), dtype=np.float32))
...
async def test_triton_grpc_client_infer_not_connected():
    ...
    with pytest.raises(RuntimeError, match="nao conectado"):
        await client.infer_symbol("OTC_SPC", np.zeros((1, 4, 34), dtype=np.float32))
