...
def test_correlation_cache_roundtrip():
    matrix = {("RDBEAR", "RDBULL"): 0.5, ("RDBULL", "RDBEAR"): 0.5}
    raw = correlation_matrix_to_cache(matrix)
    assert correlation_matrix_from_cache(raw)[("RDBEAR", "RDBULL")] == 0.5
...
async def test_read_cached_correlation_matrix():
    store = AsyncMock()
    store.get_string.return_value = json.dumps({"RDBEAR|RDBULL": 0.4})
    orch = MagicMock()
    orch.state_store = store
    matrix = await read_cached_correlation_matrix(orch)
    assert matrix[("RDBEAR", "RDBULL")] == 0.4
...
async def test_fetch_symbol_closes_and_matrix():
    ...
    with patch(
        ...
        closes = await fetch_symbol_closes("dsn", ["RDBEAR"], granularity=60, bars=3)
        matrix = await fetch_correlation_matrix("dsn", ["RDBEAR", "RDBULL"], granularity=60, bars=3)
    assert "RDBEAR" in closes
    assert ("RDBEAR", "RDBULL") in matrix
...
async def test_refresh_correlation_cache_enabled():
    ...
    with patch(
        ...
        new_callable=AsyncMock,
        return_value={("RDBEAR", "RDBULL"): 0.2},
    ):
...
async def test_infer_symbol_async_error():
    ...
    with (
        ...
        await infer_symbol_async({"infra": {"triton": {}}}, "RDBEAR", np.zeros((1, 4, 34), dtype=np.float32))
