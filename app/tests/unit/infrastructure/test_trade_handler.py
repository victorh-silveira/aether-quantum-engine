...
async def test_trade_handler_buy_via_bulk_purchase_rest(mock_ws):
    ...
    auth.rest_client.return_value.bulk_purchase = AsyncMock(
        return_value={
            ...
            "purchase_time": 1000,
            "start_time": 1000,
            "shortcode": "CALL_OTC_SPC_0.66_1000_1120_S0P_0",
            "transaction_id": "55",
        }
    )
    ...
    contract = await handler.buy_with_parameters("OTC_SPC", TradeDirection.CALL, 0.35)
...
async def test_trade_handler_bulk_purchase_requires_auth_pat_account(mock_ws):
    ...
    with pytest.raises(RuntimeError, match="AuthManager"):
        await handler.buy_with_parameters("OTC_SPC", TradeDirection.CALL, 1.0)
    auth = MagicMock()
...
async def test_trade_handler_buy_with_parameters_success(trade_handler, mock_ws):
    ...
    contract = await trade_handler.buy_with_parameters("OTC_SPC", TradeDirection.CALL, 10.0)
    assert contract.contract_id == 999
    assert contract.proposal_id == "prop-abc"
    assert contract.status == TradeStatus.OPEN
    assert contract.symbol == "OTC_SPC"
    ...
    assert proposal_req["underlying_symbol"] == "OTC_SPC"
...
async def test_trade_handler_buy_proposal_invalid_payload(trade_handler, mock_ws):
    ...
    with pytest.raises(RuntimeError, match="resposta sem proposal"):
        await trade_handler.buy_with_parameters("OTC_SPC", TradeDirection.CALL, 10.0)
...
async def test_trade_handler_buy_with_parameters_multiplier(trade_handler, mock_ws):
    ...
    contract = await trade_handler.buy_with_parameters("OTC_SPC", TradeDirection.CALL, 10.0, params=params)
    assert contract.contract_id == 999
...
def test_build_proposal_request_rise_fall():
    req = build_proposal_request("OTC_SPC", TradeDirection.PUT, 5.0, {"duration": 1, "duration_unit": "m"})
    assert req["underlying_symbol"] == "OTC_SPC"
