...
def test_kelly_calculation_standard(kelly_config):
    ...
    stake = rm.calculate_stake(1000.0, "R_75", conviction=0.6)
...
def test_kelly_negative_edge_returns_min_stake(kelly_config):
    ...
    rm = RiskManager(kelly_config)
    stake = rm.calculate_stake(1000.0, "R_75", conviction=0.5)
    assert stake == 1.0
...
def test_kelly_respects_max_stake_pct(kelly_config):
    ...
    rm = RiskManager(kelly_config)
    stake = rm.calculate_stake(1000.0, "R_75", conviction=0.8)
    assert stake == 50.0
...
def test_kelly_high_conviction_raises_stake_cap(kelly_config):
    ...
    rm = RiskManager(kelly_config)
    low = rm.calculate_stake(1000.0, "R_75", conviction=0.7)
    high = rm.calculate_stake(1000.0, "R_75", conviction=0.9)
    assert high > low
    assert high == 40.0
...
def test_kelly_dynamic_win_rate(kelly_config):
    ...
    for _ in range(5):
        ...
        rm.register_result(10.0, 1, "R_75")

    p = rm.effective_win_rate("R_75", conviction=0.6)
...
def test_kelly_respects_stake_min(kelly_config):
    ...
    rm = RiskManager(kelly_config)
    stake = rm.calculate_stake(10.0, "R_75", conviction=0.6)
    assert stake == 1.0
...
def test_kelly_intelligent_recovery(kelly_config):
    ...
    rm.active_contract_ids = [1]
    rm.register_result(-10.0, 1, "R_75")
    assert rm.pending_loss["R_75"] == 10.0

    stake_low = rm.calculate_stake(1000.0, "R_75", conviction=0.6)
    assert stake_low == pytest.approx(17.89, abs=0.1)

    stake_high = rm.calculate_stake(1000.0, "R_75", conviction=0.8)
    assert stake_high == pytest.approx(60.52, abs=0.1)

    rm.active_contract_ids = [2]
    rm.register_result(57.49, 2, "R_75")
    assert rm.pending_loss["R_75"] == 0.0
...
def test_risk_manager_consecutive_losses_reset_on_win(kelly_config):
    ...
    rm.active_contract_ids = [1]
    rm.register_result(-5.0, 1, "R_75")
    assert rm.consecutive_losses == 1

    rm.active_contract_ids = [2]
    rm.register_result(-10.0, 2, "R_75")
    assert rm.consecutive_losses == 2

    rm.active_contract_ids = [3]
    rm.register_result(15.0, 3, "R_75")
    assert rm.consecutive_losses == 0
...
def test_risk_manager_consecutive_losses_fraction_reduction(kelly_config):
    ...
    stake_base = rm.calculate_stake(1000.0, "R_75", conviction=0.6)

    # Adiciona 1 perda no símbolo R_75
    rm.active_contract_ids = [1]
    rm.register_result(-10.0, 1, "R_75")

    # Fração deve reduzir pela metade para um novo símbolo sem perdas (reduction = 0.5)
    stake_loss_1 = rm.calculate_stake(1000.0, "1HZ100V", conviction=0.6)
...
def test_risk_manager_consecutive_losses_recovery_conviction(kelly_config):
    ...
    stake_no_rec = rm.calculate_stake(1000.0, "R_75", conviction=0.65)

    # Conviction de 0.70 deve acionar recuperação bypassando a redução de tamanho
    stake_rec = rm.calculate_stake(1000.0, "R_75", conviction=0.70)
