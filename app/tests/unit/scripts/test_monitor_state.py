...
def test_resolve_session_financials_one_percent_goal():
    state = DashboardState(
        balance=9101.04,
        session_start_balance=9101.04,
        session_target_win=91.01,
        session_profit=0.0,
        compounding_rate=0.01,
    )
    fin = resolve_session_financials(state)
    assert fin.target_win == 91.01
    assert fin.target_balance == pytest.approx(9192.05, rel=1e-4)
    assert fin.remaining == 91.01
    assert "(1.0% SES. ATIVA)" in fin.goal_label
...
def test_resolve_session_financials_fallback_from_rate():
    ...
    fin = resolve_session_financials(state)
    expected_target = math.floor(9101.04 * 0.01 * 100) / 100
