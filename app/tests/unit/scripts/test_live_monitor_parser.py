...
def test_log_parser_cluster_legacy_without_first_sep():
    state = DashboardState()
    parser = LogParser(state)
    line = "[CLUSTER] M5 || R_10: CALL (Prob: 0.55 Cal: 0.60 Margin: 0.10 Edge: +0.20)"
    parser.process_line(line)
    assert state.last_telemetry["symbol"] == "R_10"
...
def test_log_parser_exec():
    ...
    parser = LogParser(state)
    line = (
        "[EXEC] || PUT [R_10] || STAKE: 2.00 (RECOVER_DAL_L1) | "
        "PEND: 1.00 | LIN: 1 | CAP: 50.00 | BANCA: 87.69 | CID: 1129497159 | PAY: 1.79"
    )
    parser.process_line(line)
    assert state.last_telemetry["symbol"] == "R_10"
    assert state.last_telemetry["dir"] == "PUT"
...
def test_log_parser_exec_legacy_without_lin_cap():
    state = DashboardState()
    parser = LogParser(state)
    line = "[EXEC] || PUT [R_10] || STAKE: 2.06 (DAL_L1) | PEND: 1.62 | BANCA: 87.69 || CID: 1129497159 | PAY: 1.79"
    parser.process_line(line)
...
def test_log_parser_ind():
    ...
    parser = LogParser(state)
    line = (
        "[IND] || RSI:  0.4859 | ADX:  0.2017 | HURST:  0.5671 | "
        "ATR:  -0.9558 | BBW:  -0.2226 | VOL_R:  1.0720 | "
        "Z:  +0.60 | ACC: 0.6433 | MARGIN: 0.120 | CAL_EDGE: -0.062 | "
        "NEUTRAL: calibrated | META_VETO: none || SCALE: tcn=PUT tape=PUT"
    )
    parser.process_line(line)
