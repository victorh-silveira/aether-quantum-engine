...
- Treino DL em velas diarias (D1 com 365 barras de historico); launch-train com `force_ok=true` **sempre exporta** TCN/meta/loss (pisos ACC/sharpness/z-IR/MAE gap desligados). Gates live (`skip_neg_edge`, FLIP, Kelly) permanecem. Com `online_training` **false**, Cal~0.52 + `SKIP:neg_edge` so muda apos **retreino + novo TorchScript**.
...
Nota operacional (**Volatility 75 (1s) M5**): pipeline: TCN CALL se Cal ≥**0.5** senao PUT → **FLIP** loss-clf se auto_learn (exit **4**, `n_train>=12`) e pe no piso → `invert_exec_side` **false** → SKIP `neg_edge` em EXPLORE (**waive** com PEND material) → Kelly / cover_l0 amort **1** → EXEC. Sem SCALE adapt / doji / exec_vs_candle / META soft Kelly. SKIP tecnico: treino/dados/deploy/predict/stop-win / cooldown / pausa. Recovery: `cover_enabled` **true**, amort **1/1**, cap L0 **3.5%**, PEND nao force-explore por near-stop, piso **1%** soberano.
