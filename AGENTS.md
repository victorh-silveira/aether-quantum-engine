...
- Sweep de horizonte **on** no launch-train (`horizon_sweep.run_in_launch_train=true`): treina **M15→H1 de 5 em 5** (15/20/…/60; treino por celula `duration=N` alinhado ao label), elege o mais assertivo com **settle_wr** ≥ be+0.03, **settle_n≥16**, historico ≥800; `quiet_train_logs` → celula **CRITICAL** + `why=` se deploy=0; pos-sweep denso (board/winner/promote/Timescale/`[META] ok`); promote copia ckpt + grava `label_horizon_bars` do winner (**nao** exporta `duration` ops)
