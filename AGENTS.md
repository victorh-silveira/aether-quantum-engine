...
Nota operacional (**Volatility 75 (1s) M5**): micro/mini em 300s; ciclo **300 s** (`require_signature_boundary` **true**, abertura M5); pipeline: SCALE vision (telemetria) → TCN sempre CALL se Cal ≥**0.5** senao PUT → **unica FLIP** se auto_learn e pe>=0.55 young / 0.58 mature (loss-clf; sem HARD SKIP/Soft / sem outros flips) → Kelly + SIDE_EQ sizing → EXEC. SKIP tecnico: treino/dados/deploy/predict/stop-win / cooldown pos-LOSS (LIN>=1: 300/300/600/900s) / pausa de sessao (3 LOSS em 5 trades, 2 ciclos). Sem `SKIP:NEUTRAL_ZONE`. **Removido:** fusao EV, signal_skip multi-gate, micro/regime/vol/exhaust/neg_edge, anti-loss EMA/RSI, invert, persistence flip. Sizing: piso Kelly **1%** banca; Single-Strike 4.31% ≈ cap 5.0% so com `live_n >= 12` e conviction ≥**0.58**; cold start Kelly × 0.40. Soft recovery: `cover_enabled` **false**; caps max_safe 3.5%. Sem revenge sizing. EMPTY tecnico = processo ok quando coerente.
