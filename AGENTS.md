...
Nota operacional (**Volatility 75 (1s) M5**): pipeline: SCALE vision → TCN CALL se Cal ≥**0.5** senao PUT → **FLIP** loss-clf se auto_learn (exit **4**) e pe no piso → SCALE **retract/explos/tape adapt** last (pode desfazer FLIP) → Kelly (META edge ≤ 0 = soft Kelly) → EXEC. SKIP tecnico: treino/dados/deploy/predict/stop-win / cooldown / pausa. Sem `cal_soft_edge` / quality gate. Edge = EV telemetria.
